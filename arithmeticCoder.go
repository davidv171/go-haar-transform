...
func (c *Coder) frequencyTableGenerator(data []uint8) {
	...
	for i := 0; i < len(data); i++ {
		...
		if c.frequencyTable[currentByte] == 0 {
			c.numberOfUniqueSymbols++
			c.readByteSequence[lastIndex] = currentByte
			lastIndex++
		}
		c.numberOfSymbols++
		c.frequencyTable[currentByte]++
	}
	c.generateHighTable()
}
...
func (c *Coder) generateHighTable() {
	prevHigh := uint32(0)
	...
	var i uint8 = 0
	for i = 0; i < c.numberOfUniqueSymbols; i++ {
		currSymbol := c.readByteSequence[i]
		currSymbolFrequency := c.frequencyTable[currSymbol]
		//We ignore 0 frequencies
		highTableEntry := prevHigh + currSymbolFrequency
		lowTableEntry := prevHigh
		c.highTable[currSymbol] = highTableEntry
		c.lowTable[currSymbol] = lowTableEntry
...
func (c *Coder) intervalCalculation(data []uint8) {
	high := c.high
	low := c.low
	quarters := c.quarters
	step := c.step
	e3Counter := c.e3Counter
	outputBits := make([]bool, 0)
	var i uint64 = 0
	for ; i < uint64(len(data)); i++ {
		step = uint32((uint64(high) - uint64(low) + 1) / uint64(c.numberOfSymbols))
		high = low + step*c.highTable[data[i]] - 1
		low = low + step*c.lowTable[data[i]]
	...
	c.low = low
	c.step = step
	c.e3Counter = e3Counter
	c.outputBits = outputBits
}
